...
> 后台路由在 `tools_manager.py:285` 的 `execute_tool`、结果注入在 `loop.py:72/109` 的
...
`communicate(timeout=120)`，`background_tasks_manager.py:123`），期间整个 agent 回合
...
2. **结果收割**：主循环每轮开头调 `collect_background_results()`（`loop.py:72`），把
...
（`loop.py:71-99`）。本类是被这两头驱动的引擎。

## 2. 数据结构（`__init__`，11-21）

| 成员 | 类型 | 用途 |
|---|---|---|
| `env` | `Env()` | 第 3 个 Env 实例（ENV.md §4）；只用 `workDirPath` 当子进程 cwd（116） |
| `tasks` | `dict[str, dict]` | 在途任务：`task_id → {tool_use_id, command, status}`；完成即被 `collect` 弹出（78） |
| `results` | `dict[str, str]` | 完成后的结果文本，键为 `task_id`；`collect` 弹出（79） |
...
| 分组 | 方法 | 行号 | 说明 |
|---|---|---|---|
| 生命周期 | `start` | 22-47 | 校验 → 登记 task → 起 daemon 线程跑 `run` → 返回 `task_id` |
| | `run` | 50-70 | 线程体：`run_bash_process` → `format_bash_result` → 写 `results`/`ready`、置 status |
| | `collect` | 72-95 | 排空 `ready`，弹 `tasks`/`results`，逐条渲染 `<task_notification>` |
| 对外薄封装 | `should_run_background` | 97-98 | 判定"是否该转后台"：`bash` 且 `run_in_background is True` |
| | `start_background_task` | 103-104 | 转发 `start` |
| | `collect_background_results` | 100-101 | 转发 `collect` |
| 进程执行 | `run_bash_process` | 106-142 | **前后台共用**：`Popen` + `communicate(120)` + 截断 50000 + 收尾收割 |
| | `format_bash_result` | 177-180 | 退出码非 0 → 前缀 `Error: command exited with status N` |
| 进程管理 | `stop_process_group` | 144-161 | POSIX `killpg(TERM→KILL)` / Windows `terminate/kill` |
| | `stop_all_shell_processes` | 163-167 | 遍历 `shell_processes` 逐个收 |
| | `handle_termination_signal` | 169-171 | SIGTERM → 收进程 → `SystemExit(128+signum)` |
| | `register_exit_handlers` | 173-175 | 注册 SIGTERM 处理 + `atexit`（构造时即调，21） |

## 4. 启动流程：start（22-47）

```
start(block)
├─ block.name != "bash" → raise Exception（23-24）        ← 只受理 bash
├─ command 取 block.input["command"]，非 str 或空 → raise ValueError（26-28）
├─ with lock:  ← 临界区
│     counter += 1；task_id = bg_{counter:04d}（31-32）
│     tasks[task_id] = {tool_use_id: block.id, command, status: "running"}（33-37）
├─ thread = Thread(target=run, args=(task_id, command), daemon=True)（39）
├─ thread.start()；失败 → 回滚 tasks.pop 再 raise（40-45）
└─ print "[background] started {task_id} {command[:60]}"（46）→ return task_id（47）
```

- **daemon 线程**（39）：主进程退出时不被后台任务吊住——配合 §8 的 `atexit` 收割；
...
```
run(task_id, command)
├─ output, exit_code = run_bash_process(command)（52）
├─ result = format_bash_result(output, exit_code)（53）
├─ status = "completed" if exit_code == 0 else "failed"（55-57）
│     ← 异常 → result = "Error: {Type}: {e}", status = "failed"（59-61）
└─ with lock:
      task = tasks.get(task_id)；None → return（64-66）
      task["status"] = status；results[task_id] = result；ready.append(task_id)（68-70）
```
...
Popen(command, shell=True, stdout=PIPE, stderr=PIPE, text=True,
      errors="replace", cwd=env.workDirPath, start_new_session=True)  ← 109-118
  shell_processes.add(process)                                          ← 120-121
  stdout, stderr = process.communicate(timeout=120)                     ← 123
  output = (stdout + stderr).strip()                                    ← 124
  ├─ output 非空 → return (output[:50000], returncode)                  ← 126-127
  └─ 空         → return ("(no output)", returncode)                    ← 129
except TimeoutExpired → ("Error: Timeout(120s)", None)                  ← 130-131
except Exception      → ("Error: {Type}: {e}", None)                    ← 132-133
finally:
  stop_process_group(process) → process.wait(0.2) → shell_processes.discard(process)  ← 134-142
```
...
## 7. 收割与通知格式：collect（72-95）

`collect()` 是**非阻塞快照**：只处理"此刻已在 `ready` 里的"，没有也不等。

```
with lock:                                        ← 74
    task_ids = list(ready); ready.clear()         ← 75-76
    for id: task = tasks.pop(id); result = results.pop(id, "")   ← 77-81   （一次性消费）
for (id, task, result) in ready:                  ← 84
    print "[background] collected {id}: {status}" ← 85
    追加 <task_notification>…                     ← 86-93
return notifications: list[str]                   ← 95
```
...
- **`result[:500]`**（91）：喂给模型的结果被截到 **500 字符**——比 `run` 里存的 50000 短一个
  数量级，是后台结果"只给个摘要"的有意收窄（对比前台 `run_bash` 可见 50000）；
- **一次性消费**：`tasks.pop` + `results.pop`（78-79）确保同一结果不被二次投递；
- **status 但非 tool_use_id**：task 里存了 `tool_use_id`（34）却**不进**通知——通知是无
  tool_use 关联的独立文本块，见 §9 为何这样设计能保持 API 配对。

## 8. 进程管理与退出收割（144-175）

- `stop_process_group`（144-161）分平台：
  - POSIX（`hasattr(os, "killpg")`）：对进程组 `killpg(SIGTERM)`，0.05s 后仍活着则
    `killpg(SIGKILL)`；`ProcessLookupError/OSError` 直接 return（已没了）；
  - Windows：无 `killpg` → `terminate()` + 0.05s + `kill()`（156-161）。⚠ 只能收割 shell
    本身，shell 再生的孙进程在 Windows 上可能残留（`start_new_session` 在 Windows 不被
    honored，见 §11.1）；
- `stop_all_shell_processes`（163-167）：快照 `shell_processes` 后逐个 `stop_process_group`；
- `register_exit_handlers`（173-175）：`__init__` 里即调用（21），注册 **SIGTERM →
...
- **立即回填的是占位文本**，不是真实结果。原始 `tool_use` 的配对在这一刻就闭合了——
  ...
  tool_use）；
- **门控三条件**（`should_run_background` 97-98 + `allow_background`）：只有 ①调用方允许
  ②工具名是 `bash` ③`input.get("run_in_background") is True` 全满足才转后台。注意
  **`is True`** 是严格布尔判定（98），模型传字符串 `"true"` 不算数，会退回前台（见 §11.4）。

## 10. 与 inject_background_results 的闭环与轮询语义（loop.py）

**谁触发收割、何时触发——这是本模块最该说清的一件事：**

- 全库只有一处调用 `collect_background_results()`：`loop.py:72` 的
  `inject_background_results`；
- 而 `inject_background_results(messages)` 只在主循环 `while True` 的**每一轮顶部**被调用一次
  （`loop.py:109`），且**在** `compactManager.prepare`（`loop.py:110`）**之前**；
- **没有计时器、没有后台轮询线程**——"轮询"粒度 = 一个 agent 回合。`collect` 非阻塞，
  有就返回、没有就返回空 list（73-74 空则直接 return）。

注入逻辑（`loop.py:76-97`）：把每条通知包成 `{"type":"text","text":…}` 块，若
`messages[-1]` 是 user 角色就 `content.extend(blocks)`（内容已是 list）或把 str 内容升级成
list 再拼（84-92）；否则新建一条 user 消息（93-97）。这与既有的 todo `<reminder>` 注入
（`loop.py:180-182`）是同一套"往最后一批 user 内容里追加文本块"的手法。

```
主循环单轮（loop.py:108-）
while True:
    inject_background_results(messages)   ← 109  收割→合并进上一条 user / 新建 user
    compactManager.prepare(...)           ← 110
    response = messages.create(...)        ← 113  模型这一轮就能看见 <task_notification>
    ... execute_tool 可能又 start 新后台任务（285）
```

**为什么是单实例（关键，曾踩坑）**：`Loop` **不再自建** `BackgroundTasksManager`，而是读
`self.toolsManager.backgroundTasksManager`（`loop.py:72`）。若两处各 new 一个，任务登记进
...
- 任务在第 N 轮的 `execute_tool` 里启动，若它在第 N+1 轮 `inject`（`loop.py:109`）之前跑完，
  则第 N+1 轮模型就能看见；没跑完则顺延到再下一轮；
- **终轮陷阱**：若模型在启动后台任务后直接给出最终答复（该轮 `tool_calls` 为空，
  `loop.py:143-153` 直接 `return` 退出 `agent_loop`），这一轮**不再回到 while 顶部**，
...
1. **全库唯一实例、且只在主线程构造**：`tools_manager.py:223` 那一个。`Loop` 经
   `toolsManager.backgroundTasksManager` 复用（`loop.py:72`），绝不另建（§10 双实例丢结果
   教训）；
...
| `loop.py` | 唯一收割方：`inject_background_results`（71-99）在 `agent_loop` while 顶部（109）调 `collect_background_results`（72），把通知并入上一条 user 消息；side-channel 调用与子代理循环不收割 |
