...
2. **结果收割**：主循环每轮开头调 `collect_background_results()`（`loop.py:76`），把
...
- 全库只有一处调用 `collect_background_results()`：`loop.py:76` 的
  `inject_background_results`；
- 而 `inject_background_results(messages)` 只在主循环 `while True` 的**每一轮顶部**被调用一次
  （`loop.py:114`），且**在** `compactManager.prepare`（`loop.py:115`）**之前**；
- **没有计时器、没有后台轮询线程**——"轮询"粒度 = 一个 agent 回合。`collect` 非阻塞，
  有就返回、没有就返回空 list（77-78 空则直接 return）。

注入逻辑（`loop.py:80-101`）：把每条通知包成 `{"type":"text","text":…}` 块，若
`messages[-1]` 是 user 角色就 `content.extend(blocks)`（内容已是 list，90-91）或把 str 内容升级成
list 再拼（92-96）；否则新建一条 user 消息（97-101）。这与既有的 todo `<reminder>` 注入
（`loop.py:187-189`）是同一套"往最后一批 user 内容里追加文本块"的手法。
...
- 任务在第 N 轮的 `execute_tool` 里启动，若它在第 N+1 轮 `inject`（`loop.py:114`）之前跑完，
  则第 N+1 轮模型就能看见；没跑完则顺延到再下一轮；
- **终轮陷阱**：若模型在启动后台任务后直接给出最终答复（该轮 `tool_calls` 为空，
  `loop.py:150-160` 直接 `return` 退出 `agent_loop`），这一轮**不再回到 while 顶部**，
