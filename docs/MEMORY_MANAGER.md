...
> 状态：完整，**已接入主循环**（`loop.py:27` 构造、`loop.py:71` 检索、
> `loop.py:114/116` 提取与合并；接线由 commit `e8046cb` 引入）
...
`MemoryManager()` 无参构造（`loop.py:27`），内部自建全部依赖：
...
每次 `agent_loop` 进入时执行一次（`loop.py:71`），产出注入系统提示的 JSON
字符串。流水线：
...
## 5. 系统提示注入（loop 侧，loop.py:29-65）

`build_system_prompt(relevant_memories)` 在 `agent_loop` 开头与检索一起
重建（`loop.py:72`），记忆相关共三段，用 `\n\n` 与前段拼接：

| 段 | 内容 |
|---|---|
| 护栏 | "Memory is selected background knowledge, not a transcript… The current user request takes priority"（loop.py:44-49）——明示记忆是数据不是指令 |
| 目录 | `Memory catalog:\n{read_memory_index()}`（loop.py:30、50）——**全库索引**，不止选中条目 |
| 记录 | `Relevant memory records:\n{load_memories 的 JSON}`（loop.py:51） |

两个行为细节（均在 loop 侧不在本类）：

- 注入的是**全文**（截断后）而非仅目录，模型无需再"点开"记忆文件；
- 无记忆时目录/记录两段仍然出现，只是内容为空（无 `if` 条件，loop.py:54-56）；
...
`loop.py:107-116`：响应**不含任何 tool_use**（即最终回答）、且 Stop hook
...
仅在本轮提取**确实写入了新记忆**时被调（`loop.py:115-116`），即合并的
...
2. **检索注入不计 CompactManager 预算**：记忆全文进的是 `system` 参数，
   `compactManager.prepare` 只管 `messages`（loop.py:74）——20000 字符
...
10. **注释里的中文翻译是残留**：415-426 与 495-499 保留着 prompt 的整段
    ...
    时容易被这些过期注释误导；
11. **loop 侧变量名拼写**：`releavant_memories`（loop.py:71-72）——不影响
    行为，跨文档检索时注意两种拼法。
...
| 模块 | 关系 |
|---|---|
| `loop.py` | 唯一调用方：构造（27）、每请求检索+重建系统提示（71-72）、最终回答后提取、提取有新增才合并（114-116）；模型无 memory 工具，ToolsManager 路由表中不存在本类任何入口 |
