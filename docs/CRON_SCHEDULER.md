...
> 对应源码：`cron_scheduler.py`（本仓库当前版本 395 行）
...
调度器实例状态（`__init__`，cron_scheduler.py:22-32）：

| 字段 | 说明 |
|---|---|
| `env = Env()` | 自建 Env 实例（:23），只为取 `durablePath`——与 loop/tools_manager 各自的 Env **不是同一实例** |
| `scheduled_jobs: dict[str, CronJob]` | 注册表（:24） |
| `cron_queue: list[CronJob]` | 待投递队列（:25），存放已触发未 ack 的 job（与注册表共享同一对象引用） |
| `cron_lock = threading.RLock()` | 保护上述两者 + 落盘（:27）；注释点明用 RLock 的原因：`save_durable_jobs` 在持有 `cron_lock` 时被调用，普通 Lock 会自死锁 |
| `runtime_stop: Event` / `runtime_started: bool` / `scheduler_loop_thread` | 后台轮询线程的生命周期控制（:29、:30、:32） |
...
| `schedule_job(cron, prompt, recurring=True, durable=True)` | — | 成功 `CronJob`；失败**错误字符串**（`CronJob \| str` 判别式返回，:205-230） | 注册表插入；durable 时 `save_durable_jobs`（失败则回滚删除并**向上抛**）；`log_info` `[cron] scheduled` |
| `cancel_job(job_id)` | — | `"Cancelled {id}"` 或 `"Job {id} not found"`（:232-253） | 从注册表和队列移除；durable 时落盘（失败回滚）；`log_info` `[cron] cancelled` |
| `new_cron_id()` | — | `"cron_xxxxxxxx"`；100 次撞车则 `ValueError`（:198-203） | 无（查重 `scheduled_jobs`，调用方已持锁） |
| `poll_due_jobs(moment)` | 判定时刻 | — | 命中的 job 经 `_enqueue_due_job` 入队并落盘；`log_info` `[cron] due`（:273-285） |
| `consume_cron_queue()` | — | 队列快照 list，**并清空队列**（:287-291） | 无落盘 |
| `acknowledge_cron_jobs(jobs)` | 已交付列表 | — | recurring → `pending_delivery=False`；one-shot → 从注册表**删除**；有 durable 变化则落盘，失败回滚（:293-334） |
| `restore_cron_jobs(jobs)` | 交付失败的列表 | — | 重新置 `pending_delivery=True` 并在不在队首时重入队；**不落盘**（:336-350，磁盘上 fire 时已写入 pending=True） |
| `has_cron_queue()` | — | `bool`（:352-354） | 无 |
| `save_durable_jobs()` / `load_durable_jobs()` | — | — | 见 §6 |
| `cron_scheduler_loop()` | — | — | 轮询主循环，见 §5 |
| `start_runtime_threads()` | — | — | 幂等（`runtime_started` 守卫，:360-368）：先 `load_durable_jobs()`，再起 daemon 轮询线程 |
| `stop_runtime_threads()` | — | — | `runtime_stop.set()` + `join(timeout=1)`（:370-376） |
| `list_cron_jobs()` | — | 注册表快照 `list[CronJob]`，每次调用新列表（元素为原 job 引用）（:378-381） | 无——外部读注册表的**唯一公开入口**，调用方不再接触 `cron_lock`/`scheduled_jobs` |
| `run_delivery(deliver)` | 投递回调 `deliver(fired)`（收 `consume_cron_queue` 返回的整批 job） | 本批大小 `int`；空批返回 `0` | at-least-once 协议的**唯一执行者**（:383-395）：`consume_cron_queue`（:386）→ 空批直接返回、不调回调（:387-388）→ 回调 `deliver(fired)`（:390）成功 → `acknowledge_cron_jobs`（:394）；回调抛**任何** `BaseException` → `restore_cron_jobs`（:392）后原样 `raise`（:393），绝不静默丢批 |

## 4. cron 表达式语义

字段顺序 `分 时 日 月 星期`，恰好 5 段否则不匹配/校验失败（:60-63、:127-129）。
支持的单字段形态（`_cron_field_matches` :40-57，`_validate_cron_field` :90-124）：
...
校验范围表 `field_rules`（:131-137）：minute 0-59、hour 0-23、day-of-month 1-31、
month 1-12、**day-of-week 0-6**。

两个星期制换算与一条经典语义：

- Python `weekday()` 周一=0，cron 惯例周日=0，故 `cron_weekday = (moment.weekday() + 1) % 7`（:67）；不支持 `7` 表示周日；
- 日/星期字段沿用 vixie-cron 语义（:79-88）：两字段都受限时按 **OR**（任一命中即触发），任一为 `*` 时按另一个字段 AND。
...
```python
def cron_scheduler_loop(self):                       # :356-358
...
- daemon 线程每 1 秒 poll 一次（`wait(1.0)` 同时充当停止信号与间隔休眠），
  ...
  去重：`pending_delivery` 已置位或 `last_fired == minute_marker` 即跳过（:274、:278）；
- **不补跑**：进程停机期间错过的分钟永久丢失；同一分钟内重启不会二次触发
  （`last_fired` 对 durable 任务已落盘）；
- `poll_due_jobs` 整体持 `cron_lock`（:275），逐 job try/except——某个 job 落盘失败
  只打日志 `[cron] could not enqueue`，不影响其余 job（:284-285）；
- 触发入队的原子序（`_enqueue_due_job` :255-271）：先置 `pending_delivery=True` 与
...
- 原子写（:151-156）：临时文件名 `<name>.<pid>.<tid>.tmp` → 写 → `os.replace`，
  `finally` 中 `unlink(missing_ok=True)` 清残留；同分区 rename 使读者永不见半截文件；
- 加载（`load_durable_jobs` :159-195）：文件缺失静默返回；坏 JSON / 非数组 →
...
- 装载时 `pending_delivery=True` 的 job 直接回灌 `cron_queue`（:191-192）——
  上一进程"已触发未确认"的任务重启后补投；
- `durable=False` 的 session 任务只存在于内存，进程退出即消失。

## 7. 投递协议：run_delivery（consume → 回调 → ack / restore）

**at-least-once** 时序由 `run_delivery`（:383-395）在调度器内部强制执行，调用方
（主循环，见 §8）只提供一个投递回调，拿不到也破坏不了协议骨架：

```
run_delivery(deliver):                  # cron_scheduler.py:383-395
    fired = consume_cron_queue()        # 整队取出并清空（:386，实现 :287-291）
    fired 为空 → return 0，不调回调      # :387-388
    try:
        deliver(fired)                  # :390 回调注入 history 并驱动 agent 回合
    except BaseException:               # :391 连 KeyboardInterrupt 也接住
        restore_cron_jobs(fired)        # :392 重新置 pending=True 并回灌队列（:336-350）
        raise                           # :393 原样上抛，绝不静默丢批
    acknowledge_cron_jobs(fired)        # :394（实现 :293-334）
    return len(fired)                   # :395
```
...
```
    ...
    └─ durable 有变化 → 落盘；落盘失败 → 恢复被删 job、恢复 pending 标志、
                       把不在队列中的 job 重新入队，再抛（:320-334）
```

- 队列防重入靠 `pending_delivery`：true 期间 poll 不会再入队（:278）、
  restore/ack 回滚时先查 `queued_ids` 再 append（:327-333、:336-350）——
  同一 job 在队列中至多一份；
...
:259 `self.cronScheduler = CronScheduler()`）。

**三个工具**（schema :212-242，注册进主循环 `tools` 列表 :277-279，handler 映射
:296-298，实现 :657-679）：
...
`subTools`（:300-313）**不含** cron 三件套——子代理不能排/查/撤定时任务，
只有主 agent 可以。
...
loop.py:256  self.cron.run_delivery(deliver)          # 投递唯一入口；deliver 闭包 :251-255：
             ...
             #   :254  log_info [cron] delivered {id}
             #   :255  _run_turn(history, "\n".join(job.prompt))  # payload 是无前缀原文
             #   协议内部：consume/ack/restore 全在 cron_scheduler.py:386/394/392，loop 不可见
...
1. **错误即值，不是异常**：`schedule_job` 用返回类型 `CronJob | str`、
   ...
   （`run_schedule_cron` :659 用 `isinstance(result, str)`），改成抛异常会同时
...
- `start_runtime_threads`/`stop_runtime_threads` 的幂等守卫是对 `runtime_started`
  的无锁裸读写（:361、:368、:371、:376），依赖"只在主线程调用一次"的约定，
  并非线程安全；
...
| `tools_manager.py` | 持有唯一实例（:259）；`schedule_cron`/`list_crons`/`cancel_cron` 三个主 agent 工具的宿主 |
