...
## 2. Task 数据结构（task_manager.py:10-19）
...
- **合法状态集**：`(pending, in_progress, completed)`，白名单校验只写在
  `load`（task_manager.py:130-131）——即"读时校验"，写路径靠状态机闸门
  （§5.4）保证不产生非法值；手工把文件改成 `"done"` 会在下次 load 抛
  `ValueError`。
- **序列化**：`asdict` + `json.dumps(indent=2)`（create 在 72、save 在
  135），文件即字段的直白映射，人肉可读可改（改坏了 load 会拒）。
- **反序列化**：`Task(**data)`（task_manager.py:126），键必须与字段**恰好
...
- **ID 格式**：`TASK_ID_PATTERN = ^task_[0-9a-f]{8}$`（task_manager.py:24），
  ID 空间 2³²；所有入口只认这个正则。
- **`_path`（task_manager.py:40-48）双层防护**：
  1. `fullmatch` 拒绝任何不合法 ID（`task_id` 甚至不是 str 也在这层挡掉，
     41-42）；
  2. 拼路径后 `resolve()` + `is_relative_to(root)`（45-46）——ID 正则已
     排除 `/`、`\`、`..`，这层是纵深防御，双保险。
- **`_root`（task_manager.py:31-37）防逃逸出工作区**：根目录 `resolve()` 后
  必须 `is_relative_to(env.workDirPath.resolve())`（34-35），否则
  `ValueError("TaskManager escapes the workspace")`。与 TOOLS_MANAGER.md
  §9 的 `safe_path` 同一思路。⚠ 第 34 行 `self.directory.resolve(...)` 的
  传参有个名不副实的坑，见 §9.2。
- **懒建目录**：`_root(create=True)` 才 `mkdir`；只有写路径（create、
  save、`_path(create_root=True)`）会建目录，读路径不产生副作用——代价是
  目录不存在时读路径直接炸（§9.2）。
- `exists`（51-52）：`_path(...).is_file()`，供依赖存在性校验。

## 4. 方法地图

| 分组 | 方法 | 行号 | 说明 |
|---|---|---|---|
| 存储层 | `exists` | 51-52 | 按 ID 判文件在否 |
| | `load` | 124-132 | 读盘 + 三校验（键一致、ID 与文件名一致、status 白名单） |
| | `save` | 134-135 | 整文件覆写 |
| | `list` | 137-147 | 目录不存在→`[]`；`sorted(glob("task_*.json"))` 逐个 load |
| 图查询 | `_depends_on` | 80-95 | task_id 的传递闭包里能否触达 target_id |
| | `incomplete_dependencies` | 164-173 | 未完成的依赖（缺失/损坏一律算未完成） |
| | `can_start` | 175-176 | 无未完成依赖 |
| 业务 | `create` | 55-77 | 生成 ID 并落盘 |
| | `update_dependencies` | 98-122 | 加边（五层校验，§5.2） |
| | `claim_task` | 180-194 | pending→in_progress |
| | `complete_task` | 198-226 | in_progress→completed + 解锁通知 |
| 工具适配 | `create_task` / `update_task` / `load_task` / `list_tasks` | 149-159 | 一行转发 |
| | `get_task` | 161-162 | load 后序列化成 JSON 字符串返回（喂模型的形态） |

149-159 的四个薄封装存在的意义是给工具层留一层命名适配（`update_task` →
`update_dependencies` 等），逻辑零附加。

## 5. 核心流程

### 5.1 create（task_manager.py:55-77）

```
subject.strip() 非空校验 → _root(create=True) 建目录
└─ for _ in range(100):                      ← 61
     task = Task(id=task_<token_hex(4)>, status=pending, owner=None, blockedBy=[])
     open("x") 独占创建文件并 json.dump       ← 71，存在即 FileExistsError
     成功 → return task；FileExistsError → 换 ID 重试
   100 次全撞 → RuntimeError("Could not allocate a unique task ID")  ← 77
```
...
### 5.2 update_dependencies（task_manager.py:98-122）：五层校验

全系统入参校验最密的函数，按序：

| # | 校验 | 行号 | 失败 |
|---|---|---|---|
| 1 | `add_blocked_by` 必须是 `list` | 99-100 | `ValueError` |
| 2 | 目标任务须 `pending` **且** `owner is None` | 103-104 | `ValueError`——已开工的任务不许改依赖 |
| 3 | 自依赖 `dependency == task_id` | 108-109 | `ValueError` |
| 4 | 依赖文件必须存在 | 111-112 | `ValueError: Dependency not found` |
| 5 | 成环检测 | 114-115 | `ValueError: Dependency cycle detected` |

细节：

- 入参先 `list(dict.fromkeys(...))` 去重、保序（106）；
- 第 5 层条件 `dependency not in task.blockedBy and self._depends_on(
  dependency, task_id)`（114）——若新边本就是重复边，跳过环检（重复添加
  无害，118 处也不会写入）；否则问"dependency 是否传递依赖 task_id"，
  是则新边成环；
- 全部通过后才追加新边并 `save`（117-121），**校验期间零副作用**。
- 小瑕疵：错误串 `"- >"` 中间多了个空格（115）。

### 5.3 _depends_on（task_manager.py:80-95）：迭代式 DFS
...
- 参数名 typo：`taks_id`（80）——纯位置传参无实害，但别当关键字传；
- 每触达一个节点都 `self.load(...)` 读盘：节点被删 → `FileNotFoundError`
  **不捕获**，直接穿透（§9.5）；
- 与 `incomplete_dependencies`（164-173）宽严不对称，见 §9.5。
...
- **claim（180-194）**：status≠pending → 返回文本 `Task {id} is {status},
  cannot claim`（183）；有未完成依赖 → `Blocked by: [...]`（187）；两者都是
  **返回文本而非抛异常**——"错误即数据"，模型能自救（换个任务claim）。
  成功则 `owner/status` 落盘、`print [claim]`、返回 `Claimed {id} {subject}`。
- **complete（198-226）**：仅 in_progress（200-201）；owner 须相符
  （203-204，该行消息有真 bug，§9.1）。

- **unblocked 通知——`ready_before` 快照对比**（207-218）：
  1. 改状态**前**，全表扫一遍，记下"pending、有依赖、且现在就能开工"的
     任务 ID 集合 `ready_before`（207-210）；
  2. 置 completed 落盘（212-213）；
  3. 再扫一遍：pending 且有依赖、**不在** `ready_before`、现在
     `can_start` → 这就是"被这次完成解锁"的任务，收集 subject（215-218）；
  4. 有则消息尾部拼 `\nUnblocked: ...` 并 `print [unblocked]`（222-224）。
...
### 5.5 incomplete_dependencies / can_start（task_manager.py:164-176）

`except (FileNotFoundError, ValueError)`（170）——依赖文件缺失或内容损坏
...
1. **ID 即文件名，永不改**：`load` 校验字段 ID 与文件名一致
   （task_manager.py:127-128）；要换 ID 只能新建；
...
1. **complete 的 owner 不符消息必崩**（task_manager.py:204）：错误消息里
...
2. **`_root` 的 `resolve()` 参数名不副实**（task_manager.py:34）：
...
3. **脏文件毒死 list()**（task_manager.py:144-145）：glob 模式
   ...
   穿透主循环。实测复现。
4. **文件 schema 漂移 → `Task(**data)` TypeError**（task_manager.py:126）：
...
5. **环检测的传递闭包会踩删号依赖**：`update_dependencies` 的存在性校验
    （111）只查**新加的直接依赖**；`_depends_on`（93）遍历传递闭包时对
    每个节点裸 `load`——若闭包深处有依赖已被手删，实测 `FileNotFoundError`
    穿透。与 `incomplete_dependencies` 的容错（170）宽严不对称，是设计
   不一致而非刻意为之。
...
8. **性能天花板**：`complete_task` 两遍全表扫（207-218），`can_start` 内
...
1. **`list` 方法遮蔽内置名，全靠 `from __future__ import annotations` 续命**
   （task_manager.py:1 / 137）：类体里第 137 行定义了方法 `list`，从这一刻
   起类体命名空间中的 `list` 不再是内置类型。其后所有 def 语句在**定义时**
   本要求值的注解——`-> list[Task]`（158）、`-> list[str]`（164）——在没有
