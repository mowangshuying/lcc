...
> 对应源码：`task_manager.py`（本仓库当前版本 227 行）
> 状态：完整，**已接入主循环**（`tools_manager.py:257` 构造；6 个工具 schema 注册于
> `tools_manager.py:271-276`、handler 路由于 `tools_manager.py:290-295`）
...
## 2. Task 数据结构（task_manager.py:11-20）
...
| `owner` | `str \| None` | `None` | 认领者；工具层恒为 `"agent"`（tools_manager.py:651/654） |
| `blockedBy` | `list[str]` | `[]` | 前置任务 ID 列表 |

- **合法状态集**：`(pending, in_progress, completed)`，白名单校验只写在
  `load`（task_manager.py:131-132）——即"读时校验"，写路径靠状态机闸门
  （§5.4）保证不产生非法值；手工把文件改成 `"done"` 会在下次 load 抛
  `ValueError`。
- **序列化**：`asdict` + `json.dumps(indent=2)`（create 在 73、save 在
  136），文件即字段的直白映射，人肉可读可改（改坏了 load 会拒）。
- **反序列化**：`Task(**data)`（task_manager.py:127），键必须与字段**恰好
...
- **ID 格式**：`TASK_ID_PATTERN = ^task_[0-9a-f]{8}$`（task_manager.py:25），
  ID 空间 2³²；所有入口只认这个正则。
- **`_path`（task_manager.py:41-49）双层防护**：
  1. `fullmatch` 拒绝任何不合法 ID（`task_id` 甚至不是 str 也在这层挡掉，
     42-43）；
     2. 拼路径后 `resolve()` + `is_relative_to(root)`（46-47）——ID 正则已
     排除 `/`、`\`、`..`，这层是纵深防御，双保险。
- **`_root`（task_manager.py:32-38）防逃逸出工作区**：根目录 `resolve()` 后
  必须 `is_relative_to(env.workDirPath.resolve())`（35-36），否则
  `ValueError("TaskManager escapes the workspace")`。与 TOOLS_MANAGER.md
  §9 的 `safe_path` 同一思路。⚠ 第 35 行 `self.directory.resolve(...)` 的
  传参有个名不副实的坑，见 §9.2。
- **懒建目录**：`_root(create=True)` 才 `mkdir`；只有写路径（create、
  save、`_path(create_root=True)`）会建目录，读路径不产生副作用——代价是
  目录不存在时读路径直接炸（§9.2）。
- `exists`（52-53）：`_path(...).is_file()`，供依赖存在性校验。

## 4. 方法地图

| 分组 | 方法 | 行号 | 说明 |
|---|---|---|---|
| 存储层 | `exists` | 52-53 | 按 ID 判文件在否 |
| | `load` | 125-133 | 读盘 + 三校验（键一致、ID 与文件名一致、status 白名单） |
| | `save` | 135-136 | 整文件覆写 |
| | `list` | 138-148 | 目录不存在→`[]`；`sorted(glob("task_*.json"))` 逐个 load |
| 图查询 | `_depends_on` | 81-96 | task_id 的传递闭包里能否触达 target_id |
| | `incomplete_dependencies` | 165-174 | 未完成的依赖（缺失/损坏一律算未完成） |
| | `can_start` | 176-177 | 无未完成依赖 |
| 业务 | `create` | 56-78 | 生成 ID 并落盘 |
| | `update_dependencies` | 99-123 | 加边（五层校验，§5.2） |
| | `claim_task` | 181-195 | pending→in_progress |
| | `complete_task` | 199-227 | in_progress→completed + 解锁通知 |
| 工具适配 | `create_task` / `update_task` / `load_task` / `list_tasks` | 150-160 | 一行转发 |
| | `get_task` | 162-163 | load 后序列化成 JSON 字符串返回（喂模型的形态） |

150-160 的四个薄封装存在的意义是给工具层留一层命名适配（`update_task` →
`update_dependencies` 等），逻辑零附加。

## 5. 核心流程

### 5.1 create（task_manager.py:56-78）

```
subject.strip() 非空校验 → _root(create=True) 建目录
└─ for _ in range(100):                      ← 62
     task = Task(id=task_<token_hex(4)>, status=pending, owner=None, blockedBy=[])
     open("x") 独占创建文件并 json.dump       ← 72，存在即 FileExistsError
     成功 → return task；FileExistsError → 换 ID 重试
   100 次全撞 → RuntimeError("Could not allocate a unique task ID")  ← 78
```
...
### 5.2 update_dependencies（task_manager.py:99-123）：五层校验

全系统入参校验最密的函数，按序：

| # | 校验 | 行号 | 失败 |
|---|---|---|---|
| 1 | `add_blocked_by` 必须是 `list` | 100-101 | `ValueError` |
| 2 | 目标任务须 `pending` **且** `owner is None` | 104-105 | `ValueError`——已开工的任务不许改依赖 |
| 3 | 自依赖 `dependency == task_id` | 109-110 | `ValueError` |
| 4 | 依赖文件必须存在 | 112-113 | `ValueError: Dependency not found` |
| 5 | 成环检测 | 115-116 | `ValueError: Dependency cycle detected` |

细节：

- 入参先 `list(dict.fromkeys(...))` 去重、保序（107）；
- 第 5 层条件 `dependency not in task.blockedBy and self._depends_on(
  dependency, task_id)`（115）——若新边本就是重复边，跳过环检（重复添加
  无害，119 处也不会写入）；否则问"dependency 是否传递依赖 task_id"，
  是则新边成环；
- 全部通过后才追加新边并 `save`（118-122），**校验期间零副作用**。
- 小瑕疵：错误串 `"- >"` 中间多了个空格（116）。

### 5.3 _depends_on（task_manager.py:81-96）：迭代式 DFS
...
- 参数名 typo：`taks_id`（81）——纯位置传参无实害，但别当关键字传；
- 每触达一个节点都 `self.load(...)` 读盘：节点被删 → `FileNotFoundError`
  **不捕获**，直接穿透（§9.5）；
- 与 `incomplete_dependencies`（165-174）宽严不对称，见 §9.5。
...
- **claim（181-195）**：status≠pending → 返回文本 `Task {id} is {status},
  cannot claim`（184）；有未完成依赖 → `Blocked by: [...]`（188）；两者都是
  **返回文本而非抛异常**——"错误即数据"，模型能自救（换个任务claim）。
  成功则 `owner/status` 落盘、打 `[task] claim ...` 日志、返回 `Claimed {id} {subject}`。
- **complete（199-227）**：仅 in_progress（201-202）；owner 须相符
  （204-205，该行消息有真 bug，§9.1）。

- **unblocked 通知——`ready_before` 快照对比**（208-219）：
  1. 改状态**前**，全表扫一遍，记下"pending、有依赖、且现在就能开工"的
     任务 ID 集合 `ready_before`（208-211）；
  2. 置 completed 落盘（213-214）；
  3. 再扫一遍：pending 且有依赖、**不在** `ready_before`、现在
     `can_start` → 这就是"被这次完成解锁"的任务，收集 subject（216-219）；
  4. 有则消息尾部拼 `\nUnblocked: ...` 并打 `[task] unblocked ...` 日志（223-225）。
...
### 5.5 incomplete_dependencies / can_start（task_manager.py:165-177）

`except (FileNotFoundError, ValueError)`（171）——依赖文件缺失或内容损坏
...
|---|---|---|---|---|
| `create_task` | 144-157 | 614-617 | `subject`，`description?` | `Created {id}: {subject}` + `[task] create` 日志 |
| `update_task` | 158-174 | 619-623 | `task_id`、`addBlockedBy` | `Updated {id} blockedBy: ...` + `[task] update` 日志 |
| `list_tasks` | 176-181 | 625-645 | 无 | `[ ]/[>]/[x] {id}: {subject} [{status}] [owner] (blockedBy: ...)`；空→`No tasks. Use create_task to add some.` |
| `get_task` | 182-191 | 647-648 | `task_id` | 任务全文 JSON 字符串 |
| `claim_task` | 192-201 | 650-651 | `task_id` | claim 的文本（含 Blocked by） |
| `complete_task` | 202-211 | 653-654 | `task_id` | complete 的文本（含 Unblocked） |

- **登记三处同步**（TOOLS_MANAGER.md §10 不变量 1 在 task 上的体现）：
  schema 类属性 + `self.tools`（271-276，经 `*_info()` 转发方法
  390-407 取值）+ `toolsHandlers`（290-295）；
- **子代理不可见**：6 个 task 工具都不在 `subTools`（300-306），看板只归
  主代理管；
- `owner` 在工具层写死 `"agent"`（651、654）——当前没有多 agent 概念，
  owner 字段是为将来预留的；
- `run_*` 封装全部**无 try/except**：claim/complete 的"业务失败"以文本
  ...
  `FileNotFoundError` 会穿透 `execute_tool` 的裸调用
   （tools_manager.py:349）炸穿主循环（TOOLS_MANAGER.md §11.1 同款，
  详见 §9）；
- **参数名即契约**：`handler(**block.input)` 靠形参名匹配 schema 属性名，
   `update_task` 的 camelCase `addBlockedBy`（165 与 619）必须逐字一致；
...
| 项 | 值 | 位置 |
|---|---|---|
| ID 正则 | `^task_[0-9a-f]{8}$` | task_manager.py:25（UPDATE_TASK schema 里的 `pattern` 是它的镜像，tools_manager.py:164/167） |
| ID 随机源 | `secrets.token_hex(4)` | task_manager.py:63 |
| create 重试 | 100 次 | task_manager.py:62 |
| 合法状态 | `pending/in_progress/completed` | task_manager.py:131 |
...
1. **complete 的 owner 不符消息必崩**（task_manager.py:205）：错误消息里
...
2. **`_root` 的 `resolve()` 参数名不副实**（task_manager.py:35）：
...
5. **环检测的传递闭包会踩删号依赖**：`update_dependencies` 的存在性校验
     （112）只查**新加的直接依赖**；`_depends_on`（94）遍历传递闭包时对
    每个节点裸 `load`——若闭包深处有依赖已被手删，实测 `FileNotFoundError`
    穿透。与 `incomplete_dependencies` 的容错（171）宽严不对称，是设计
   不一致而非刻意为之。
...
8. **性能天花板**：`complete_task` 两遍全表扫（208-219），`can_start` 内
...
1. **`list` 方法遮蔽内置名，全靠 `from __future__ import annotations` 续命**
   （task_manager.py:1 / 138）：类体里第 138 行定义了方法 `list`，从这一刻
   起类体命名空间中的 `list` 不再是内置类型。其后所有 def 语句在**定义时**
   本要求值的注解——`-> list[Task]`（159）、`-> list[str]`（165）——在没有
...
| 模块 | 关系 |
|---|---|
| `tools_manager.py` | 唯一消费方：`tools_manager.py:12` 导入、`tools_manager.py:257` 以 `env.taskDirPath` 构造；6 schema（144-211）、`self.tools` 注册（271-276）、`toolsHandlers` 路由（290-295）、`run_*` 封装（614-654）；子代理 `subTools`（300-306）不含 task |
